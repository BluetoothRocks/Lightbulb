(function() {
  ...
  class BluetoothBulb {
      ...
      return new Promise((resolve, reject) => {
        navigator.bluetooth
          .requestDevice({
            ...
            optionalServices: BULBS.map(
              i => i.optionalServices
          ...
          .then(device => {
            ...
            this._BULB = BULBS.filter(item => {
              return item.filters.filter(
                filter =>
                  filter.namePrefix &&
                  device.name.indexOf(filter.namePrefix) === 0
              ).length;
            }).pop();
    ...
    set color(color) {
      ...
      this._SERVER
        .getPrimaryService(this._BULB.write.service)
        .then(service => {
    ...
    _retrieveColor() {
      return new Promise((resolve, reject) => {
        if (this._BULB.read) {
          this._SERVER
            .getPrimaryService(this._BULB.read.service)
            .then(service => {
        ...
        if (this._BULB.notify) {
          this._SERVER
            .getPrimaryService(this._BULB.notify.listen.service)
            .then(service => {
              return service.getCharacteristic(
                this._BULB.notify.listen.characteristic
              );
            })
            .then(characteristic => {
              ...
              characteristic.addEventListener(
                ...
                event => {
                  resolve(
                    this._BULB.notify.listen.interpret(event.target.value)
                  );
                }
              );
              ...
              this._SERVER
                .getPrimaryService(this._BULB.notify.write.service)
                .then(service => {
                  return service.getCharacteristic(
                    this._BULB.notify.write.characteristic
                  );
                })
                .then(characteristic => {
                  return characteristic.writeValue(
                    this._BULB.notify.write.payload
                  );
                })
                .catch(error => {
                  console.log("Argh! " + error);
