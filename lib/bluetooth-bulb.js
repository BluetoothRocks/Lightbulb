		...
		connect() {
            ...
            return new Promise((resolve, reject) => {
	            ...
	            navigator.bluetooth.requestDevice({
		            filters: BULBS.map(item => item.filter),
		            ...
		            .then(device => {
		                ...
		                this._BULB = BULBS.filter(item => {
			                if (item.filter.namePrefix && device.name.indexOf(item.filter.namePrefix) === 0) return true;
		                }).pop();
